...
func death():
	...
	await anim_feedback.animation_finished
	fade_out_audio(get_node("AudioStreamPlayer"), 2.0)

func fade_out_audio(audio_player, duration):
	var start_volume = audio_player.volume_db
	var target_volume = -80.0  # El volumen más bajo (silencio)
	var elapsed_time = 0.0

	while elapsed_time < duration:
		elapsed_time += get_process_delta_time()
		var t = elapsed_time / duration
		audio_player.volume_db = lerp(start_volume, target_volume, t)
		await get_tree().create_timer(0.01).timeout

	audio_player.stop()  # Detenemos el audio una vez que el volumen llega a cero
	audio_player.volume_db = start_volume  # Restauramos el volumen original
